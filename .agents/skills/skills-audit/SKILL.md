...
- `testing` vs `local-testing` — test design and local validation are different workflows.
- `react` vs `spa-routes` — UI component conventions and route ownership are different decisions.
- `microcopy` vs UI/component skills — content quality and implementation mechanics are different surfaces.
...
For narrow domain skills (e.g. one-off CLI workflows or a specific runtime surface):

```bash
# Confirm the referenced code surface still exists
rg -l "TracePanel|HermesApiClient|src-tauri|router.tsx" src src-tauri # adjust per skill
